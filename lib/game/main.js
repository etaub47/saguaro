...
.defines(function(){
    ...
    MyGame = ig.Game.extend({
		...
		draw: function() {
			...
			if (this.showSpeech) {
				...
				if (this.talkingShop > 0) {
					shopItems = this.shop.getItems(this.talkingShop);
					shopPosY = 50;
					for (var shopIndex = 0; shopIndex < shopItems.length; shopIndex++) {
						shopItems[shopIndex].shopImage.draw(20, shopPosY);
		...
		update: function() {
			if (ig.input.pressed('inventory') && this.pauseTimer.delta() > 0.3) {
				this.showStats = !this.showStats;
				this.pauseTimer.reset();
			}
			else if (ig.input.pressed('talk') && this.talking && this.pauseTimer.delta() > 0.3) {
				this.showSpeech = !this.showSpeech;
				this.pauseTimer.reset();
			}
			else if (this.showSpeech) {
				if (ig.input.pressed('down') && this.talkingShopIndex < this.shop.getItems(this.talkingShop).length - 1)
					this.talkingShopIndex++;
				else if (ig.input.pressed('up') && this.talkingShopIndex > 0)
					this.talkingShopIndex--;
				else if (ig.input.pressed('buy')) {
					cost = this.shop.getItems(this.talkingShop)[this.talkingShopIndex].cost;
					if (cost > this.stats.gold) {
						// TODO: play error sound
						this.talkingMessage = "You don't seem to have enough gold.";
					}
					else if (false) { // TODO: change this to check if the player already has the item
						// TODO: play error sound
						this.talkingMessage = "You already have one, don't you?";
					}
					else {				
						this.buySFX.play();
