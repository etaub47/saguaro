...
.defines(function(){
    ...
    MyGame = ig.Game.extend({
		...
		draw: function() {
			...
			var lifeOffsetX = 15;
			for (var i = 0; i < Math.floor(this.stats.hpCurrent / 5); i++) {
				this.lifeSpriteFull.draw(lifeOffsetX, 2);
				lifeOffsetX += this.lifeSpriteFull.width;
			}
			for (var i = 0; i < Math.floor((this.stats.hpMax - this.stats.hpCurrent) / 5); i++) {
				this.lifeSpriteEmpty.draw(lifeOffsetX, 2);
				lifeOffsetX += this.lifeSpriteEmpty.width;
			}
			this.hudFont.draw("MP", 2, 12);            
			var magicOffsetX = 15;
			for (var i = 0; i < Math.floor(this.stats.mpCurrent / 5); i++) {
				this.magicSpriteFull.draw(magicOffsetX, 11);
				magicOffsetX += this.magicSpriteFull.width;
			}
			for (var i = 0; i < Math.floor((this.stats.mpMax - this.stats.mpCurrent) / 5); i++) {
				this.magicSpriteEmpty.draw(magicOffsetX, 11);
				magicOffsetX += this.magicSpriteEmpty.width;
			}
			...
			this.crystalSprite.draw(240, 2);
			this.hudFont.draw(this.stats.crystals, 250, 2);
			this.beerSprite.draw(240, 11);
			this.hudFont.draw(this.stats.beer, 250, 12);
			this.antidoteSprite.draw(280, 2);
			this.hudFont.draw(this.stats.antidote, 290, 2);
			this.goldSprite.draw(280, 11);
			this.hudFont.draw(this.stats.gold, 290, 12);
            this.hudFont.draw("LV", 320, 2);
            this.hudFont.draw(this.stats.level, 335, 2);
            this.hudFont.draw("XP", 320, 11);
            this.hudFont.draw(this.stats.xpCurrent, 335, 11);
			if (this.showSpeech) {
				...
				var y = 82;
				for (var s = 1; s <= this.skills.getNumSkills(); s++) {
                    var skill = this.skills.getSkill(s);
                    var skillLevel = this.stats.getSkillLevelById(s);
                    if (!skill.isSpell() && skillLevel > 0) {
                        var name = (skill.shortcut == '-') ? skill.name : skill.name + " (" + skill.shortcut + ")";
                        this.freeSans12.draw(name, 150, y);
                        this.freeSans12.draw(skillLevel + "/" + skill.maxLevel, 265, y);
                        y += 12;
                    }
                }
				...
				for (var s = 1; s <= this.skills.getNumSkills(); s++) {
                    ...
                    var skillLevel = this.stats.getSkillLevelById(s);
                    if (skill.isSpell() && skillLevel > 0) {
                        this.freeSans12.draw(skill.name, 335, y);
                        this.freeSans12.draw(skillLevel + "/" + skill.maxLevel, 414, y);
                        this.freeSans12.draw(skill.mpCost + " MP", 454, y);
                        y += 12;
                    }
                }
			}
