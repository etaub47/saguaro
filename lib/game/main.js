    ...
    MyGame = ig.Game.extend({
		...
		draw: function() {
			...
			if (this.talking) {
				...
				if (this.talkingTimer.delta() > 0.1 && !this.showSpeech)
					this.talking = false;
			}
		...
		update: function() {
			if (ig.input.state('inventory') && this.pauseTimer.delta() > 0.3) {
				this.showStats = !this.showStats;
				this.pauseTimer.reset();
			}
			else if (ig.input.state('talk') && this.talking && this.pauseTimer.delta() > 0.3) {
